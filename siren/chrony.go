...
func NTP(w, f int) Status {
	out, err := exec.Command("chronyc tracking | grep "Last offset" | awk '{print $4}'").Output()
	if err != nil {
	...
	fmt.Fprintf(os.Stdout, "CHRONY Offset", out)
	fmt.Fprintf(os.Stdout, "CHRONY Float", floatStr)
